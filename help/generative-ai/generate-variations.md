...
Above each set of variations is the prompt that created them, along with a **Re-use** option. If you ever need to re-run a prompt with its inputs, select **Re-use** and they will reload in **Inputs**.
...
>[!NOTE]
>
>**Generate Image** will only be available if you have an Adobe Express entitlement as part of your IMS organization, and access granted to you in the Admin Console.
...
When complete select the desired image and **Save**. Adobe Express will close and the image returned, and saved with the variation.
...
* **Copy**: [will copy the image to the clipboard for use elsewhere](#use-content)
* **Edit**: will open Adobe Express so that you can make changes to the image
* **Download**: will download the image to your local machine
* **Delete**: will remove the image from the variation
...
This section provides links to your recent activity. A recent will be added after you select **Generate** with the name of the prompt and a time stamp. If you select a link it will load the prompt, populate the input fields as appropriate, and show generated variations.

## Edit the Prompt {#edit-the-prompt}

The underlying prompt can be edited. You may want to do this:

* If the generated results you are getting need further refinement
...
This will open the prompt editor where you can make your changes:

![Generate Variations - prompt editor](assets/generate-variations-prompt-editor.png)

### Add Prompt Inputs {#add-prompt-inputs}

When creating a new prompt or editing a prompt, you may want to add input fields. Input fields act as variables in the prompt and give the flexibility to use the same prompt in various scenarios. They allow users to define specific elements of the prompt, without having to write the entire prompt.
...
1. Create a new spreadsheet named pagetype.xls in the top-level directory of your folder structure.
...
When you have edited the prompt the **Save** option will be available at the bottom of the Inputs section, to the left of **Generate**.

When selected, the **Save Prompt** dialog will open:
...
1. Add a unique **Prompt Name**; used to identify the prompt within **Prompt Templates**.
   1. A new, and unique, name will create a new prompt template.
   1. An existing name will overwrite that prompt, a message will be shown.
...
>[!NOTE]
>
>You will be informed (warned) if you are overwriting/updating an existing prompt.
...
1. Then select the required **Target Audience**, from the list provided.
   ...
   >[!NOTE]
   >
   >To use an **Adobe Target** audience the description field must be filled in. If not, the audience will show in the drop down list as unavailable. To add a description go to Target and [add an audience description](https://experienceleague.adobe.com/docs/target-learn/tutorials/audiences/create-audiences).
...
See [Create audiences](https://experienceleague.adobe.com/en/docs/target-learn/tutorials/audiences/create-audiences) to create a new audience in Adobe Target.
...
You can add a CSV file from various platforms (for example, Google Drive, DropBox, Sharepoint etc.) that have the capability to provide a URL to the file once it is made publicly available.
...
1. You'll find 1 or 2 requirements that talk about the output.
...
1. Open the actual prompt in edit mode.
1. Go to requirements.
1. You'll find that for each output there will be a corresponding word / character limit.
...
One generation of a copy variant is equal to one generative action. Customers will have a certain number of generative actions that come with their AEM license. Once the base entitlement is consumed, customers will have the ability to purchase additional actions.
...
* When generating images the prompt in Adobe Express is now pre-populated
...
Moving to the v2.0.0 release will cause custom prompt templates to break - so they will be unavailable. To retrieve them:
