...
Apache Jackrabbit Oak enables flexible index configurations to efficiently handle search queries. While index optimizations may not play a major role for small to medium-sized projects, it is imperative for projects with large content repositories and higher content velocity to carry out targeted efficiency improvements for indexing. Non-optimized indexes and fallback indexes should be avoided as much as possible. It is recommended to take proactive steps to ensure that suitable and optimized indexes are available for all of your queries in AEM. In the absence of a suitable index, queries traverse the entire repository - such queries should be identified by analysing the log files in order to optimise the index definitions accordingly since a repository traversal query is the least efficient query method in AEM. Please see [this page](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/practices/best-practices-for-queries-and-indexing.html?lang=en#tips-for-creating-efficient-indexes)  for more information.

### Lucene full text index on AEM as a Cloud Service

The full-text index lucene2, indexes all content in the AEM repository by default and is therefore extremely inefficient due to its repository-dependent size. The Lucene full-text index has been internally deprecated and will no longer be deployed in AEM as a Cloud Service from September 2021. As such, it is no longer used on the product side in AEM as a Cloud Service and it should not be required to run customer code. For AEM as a Cloud Service environments with common Lucene Indexes, Adobe is working with customers individually for a coordinated approach to compensate for this index and to use better, optimized indexes. If, contrary to all expectations, a full-text index is actually required to perform queries in custom code, the index definition analog to Lucene index should be created under a different name to avoid conflicts in maintenance.
