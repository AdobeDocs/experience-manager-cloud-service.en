...
## Generate a project {#generate-project}
...
1. Run the following Maven command to generate a new project in [batch mode](https://maven.apache.org/archetype/maven-archetype-plugin/examples/generate-batch.html).
...
## Introduction to Client Libraries {#introduction-to-client-libraries}
...
The way we can apply brand specific styles to AEM CIF Core Components is by adding and overriding the CSS managed by these client libraries. Understanding how client libraries are structured and included on the page is critical.
...
1. **clientlib-base** - This is an empty client library that simply embeds the necessary dependencies from [AEM Core Components](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html).
...
1. **theme** - This is the client library that contains the starting theme and CSS for the CIF project. This client library is designed to be customized by each implementation, and it is helpful to start with some existing styles so that the components are functional to start with.
...
1. Beneath the **theme** client library you should see a file named **css.txt** located at: `ui.apps/src/main/content/jcr_root/apps/acme/clientlibs/theme/css.txt`:
...
1. Inspect the CSS files beneath the **theme/components** to get an idea of the ootb component styles. We will update some of these files later in the tutorial.
...
1. Navigate into the **Acme Store Configuration** folder. Open the **Category Page Template** by selecting the *pencil* icon.
...
1. This will open the Page Policy for the Catalog Page template:
...
1. View the page source and you should be able to identify the following client libraries are included: **acme.wcm.base**, **wcm.foundation.components.page.responsive**, **acme.theme**, **core.cif.components.react**
...
Now that we understand the client library structure generated by the archetype, we can begin to customize the AEM CIF Core Components. We will modify the styles of the Product Teaser component so that it looks more like a "card".
...
### Update the CSS for the Product Teaser {#update-css-product-teaser}
...
1. Open the file **teaser.css** and update the corresponding CSS rules (or download the [solution teaser.css file](/help/commerce-cloud/assets/style-cif-component/solution-teaser.css) and replace).
...
1. Deploy the updates of the **ui.apps** module to AEM using your Maven skills, from a command line terminal:
...
1. Experiment by adding additional Product teasers. Use Layout Mode to change the width and offset of the components in order to display multiple teasers in a row.
    ...
    >[!NOTE]
    >Each product teaser contains the same style.
...
You just styled your first AEM CIF Core Component!
