...
* Note that there are a few different ways of creating users and group in IMS.  See [IMS Support for Adobe Experience Manager as a Cloud Service ](/help/security/ims-support.md) to learn about all the options available.  Only the Admin Console bulk upload methods are described here
* There are three Identity Types possible in IMS: **Adobe ID**, **Enterprise ID**, and **Federated ID**. The instructions on this page are provided for **Adobe ID only**.  If you need to use Enterprise ID or Federated ID, please refer to the full [Admin Console documentation](https://helpx.adobe.com/enterprise/using/admin-console.html) and also the specific documentation for [Admin Console Bulk Group upload](https://helpx.adobe.com/enterprise/using/user-groups.html) and [Admin Console Bulk User upload](https://helpx.adobe.com/enterprise/using/bulk-upload-users.html).  The specifications for the upload files are somewhat different for those two Identity Types
