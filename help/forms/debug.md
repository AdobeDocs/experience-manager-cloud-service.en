...
1. Open the logs to identify the type of error. For detailed information on logs, see [logs for HTML5 forms](/help/forms/enable-logs.md).
...
1. Enable the client side logs as mentioned at [Logs for the HTML5 forms](/help/forms/enable-logs.md) using debug option as **1-a5-b5-c5**. Then render the form and click submit. Open browser debug console and check if there is an error.
1. Locate the server logs as mentioned at [Logs for the HTML5 forms](/help/forms/enable-logs.md). Check if there was any error in server logs during submission.
