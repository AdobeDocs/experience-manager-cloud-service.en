...
If your existing Cloud Service environment provides the option to [create Core Components-based Adaptive Forms](creating-adaptive-form-core-components.md), Adaptive Forms Core Components and Headless Adaptive Forms are already enabled for your environment.
...
1. The verification check in the [Check if Adaptive Forms Core Components are enabled](#check-if-enabled) section confirms that the required dependencies are missing from your repository

If you're unsure, follow the verification steps in the [Check if Adaptive Forms Core Components are enabled](#check-if-enabled) section above.
