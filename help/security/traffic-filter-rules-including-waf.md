...
| CMDEXE  | Command Execution  | Command Execution is the attempt to gain control or damage a target system through arbitrary system commands by user input.  |
...
This rule blocks requests to path `/block-me`, and blocks every request that matches a `SQLI` or `XSS` pattern. This example includes a WAF traffic filter rule, which references the `SQLI` and `XSS` [WAF Flags](#waf-flags-list), and thus requires a separate license.
...
Block requests on path /critical/resource for 60 seconds when it exceeds an average of 100 req/sec (per CDN POP) in the last 60 seconds:
...
[Work through a tutorial](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/security/traffic-filter-and-waf-rules/overview) to gain practical knowledge and experience around traffic filter rules.
