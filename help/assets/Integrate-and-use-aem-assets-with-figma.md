...
description: Learn to integrate [!DNL AEM Assets] with [!DNL Figma] and use assets from [!DNL AEM Assets] in [!DNL Figma] through an asset selector panel in [!DNL Figma].  
hide: no
role: User
---
...
Integrate [!DNL AEM Assets] with [!DNL Figma] to get an asset selector panel in [!DNL Figma]. Use this asset selector panel to access to the [!DNL AEM Assets] and use them in your design workflow in [!DNL Figma].
...
1. Access to the following Assets Cloud Service environments: 
    1. Assets Essentials UI
    1. Assets enterprise Cloud Service environment
    1. Assets Prime or Ultimate licence

# AEM Assets for Figma - v0.0.13

**NOTE:** This is the author urls build
...
- Switch to Author URLs for asset image import

## Instructions for running/publishing plugin
### How to Run Plugin Locally

1. Install figma desktop application from https://www.figma.com/downloads
2. Create or log into your figma account inside the app
3. Open any figma document
4. In the application's tool bar select "**Plugins > Development > Import plugin from manifest...**" and open the **manifest.json** file from this folder.
5. The plugin manager will open (if it doesn't you can navigate there via "**Plugins > Development > Manage plugins in development**")
6. From the plugin manager, select AEM Asset Selector to run the plugin

### How to Publish Plugin Privately to a Figma Organization

1. Follow the steps from "How to Run Plugin Locally" above in order to import the plugin to the Figma Desktop app
2. Open any document that is owned by your Figma Organization
3. Navigate to the plugin managaer. You can get there from the tool bar via "**Plugins > Development > Manage plugins in development**"
4. Find the plugin you want to publish and click on the "..." which will show when you hover over it
5. Select publish to open the "Publish plugin" screen
6. Enter the plugin information as needed
7. **IMPORTANT**: Make sure to select your organization under the "Publish to" option so that the plugin is private to the organization
8. Once all required data is filled, select "Publish" to publish the plugin


## AEM Cloud Manager & IMS Setup

Currently, there a few steps required to enable the plugin's access to a given AEM repo, even if the user's account has access to the repo already.

Please check the following for the Cloud Manager / IMS Org settings:
1. Adobe IMS is provisioned with the clientID and redirect url for your org:
    * Client ID - aemcs-figma-integration-assetselector
    * Redirect URL - https://26196-assetselectoroauth-stage.adobeioruntime.net/api/v1/web/asset-selector-oauth-service/oauthCallback
2. The environment variable ADOBE_PROVIDED_CLIENT_ID is set in Cloud Manager:
    * value is the Client ID - aemcs-figma-integration-assetselector

### Adjustments for CORS

Additonally CORS settings may need to be adjust for the AEM repo to enable export functionality. The following CORS headers must be present:
* `access-control-allow-headers` must include `CSRF-Token` and `x-api-key`
    * Example of valid header: `access-control-allow-headers: Authorization, Origin, Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers, x-api-key, CSRF-Token`
* `access-control-allow-origin` must include `null` since the plugin environment has a null origin
    * Example of valid header: `access-control-allow-origin: *`


## Known Issues/Limitation as of 2025-03-20

### A. Importing Feature (Asset Selector)
1. Disabling the selectable mime types is not exhaustive, so some unsupported mimetypes may be selectable
2. Unapproved assets are selectable but may not be importable and may show a 403 error message if import is attempted

### B. Auth Issues
1. When user's access token expires (after 24 hours), user is automatically logged out. Can improve this behaviour with token refresh feature.

# AEM Assets for Figma - v0.0.12

**NOTE:** This is the Dynamic Media Open API build

## CHANGE LOG

### v0.0.12 & v0.0.11 & v0.0.10

Features:

- Drag & Drop Functionality
- Improve download message UI

Fixes:

- Show error message on upload options modal when there is no figma content available to preview or export
