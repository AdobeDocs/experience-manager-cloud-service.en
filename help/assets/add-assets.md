...
* [!DNL Experience Manager Assets] now supports folders containing large numbers of child assets. Once a folder contains over 1000 direct children (either assets or subfolders) the Admin UI uses an asynchronously updated index to list the folder contents. As such, there might be a short delay in the visibility of newly created folders and assets (generally just a few seconds) and, when opening such a folder in the Admin View, a banner is shown stating the following - “This directory contains 1000+ items. Uploads and new folder creations may be delayed.” - to notify end users of this behaviour.
