...
* [!DNL Experience Manager Assets] can now support folders containing large numbers of child assets. Once a folder contains over 1000 assets, the Admin UI uses an asynchronously updated index to list the folder contents, and there might be a short delay in the visibility of newly created folders and assets (generally just a few seconds). When opening such a folder in the Admin View a banner is shown stating the following - “This directory contains 1000+ items. Uploads and new folder creations may be delayed.”.
