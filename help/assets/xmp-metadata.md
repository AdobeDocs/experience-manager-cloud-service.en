...
The **[!UICONTROL com.day.cq.dam.commons.metadata.XmpFilterBlackWhite]** OSGi component that controls which XMP metadata properties are filtered out of the Digital Asset Management (DAM) assets is supported in AEM as a cloud service. Note that the configuration property name changed from **[!UICONTROL xmp.filter.blacklist]** (AEM 6.5) to **[!UICONTROL xmp.filter.blocklist]** (AEMaaCS). Using the old property name silently has no effect. Deploy the OSGi configuration as usual and verify the applied values in the developer console after deployment.
