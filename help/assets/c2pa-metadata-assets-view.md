---
title: C2PA metadata integration
description: C2PA metadata, integrated into AEM Assets and featured within the Assets View, can offer context into the history of an asset, including how it was made and who was involved in creating it. Like a nutrition label for digital content, C2PA metadata can help increase transparency and build trust with audiences.
role: User
...
Brands are more concerned than ever about content transparency, AI disclosure, and preventing the tampering of assets. The Content Authenticity Initiative (CAI) at Adobe builds tools compliant with the [Coalition for Content Provenance and Authenticity](https://c2pa.org/specifications/specifications/1.1/specs/C2PA_Specification.html#_trust_model) (C2PA) technical standard. C2PA metadata, which is a new kind of encrypted, tamper-evident metadata can help viewers understand the lineage of content and ensure the integrity of brand assets. They can include a wide range of provenance data that offer insight into the history of a digital asset.
...
* **Issue Date:** The date on which C2PA metadata was applied to the asset. 
...
Adobe Experience Manager Assets now supports C2PA metadata, enabling users to see C2PA metadata directly within the Assets view of AEM. When looking at the asset details, any image with C2PA metadata (such as those created with GenAI services) shows the manifest details in a dedicated panel. If the asset is downloaded, published, or shared, the C2PA metadata remains intact with the asset.

![assets](/help/assets/assets/content-credentials.png)

## Access C2PA metadata {#access-C2PA-metadata}  

1. Go to Assets View UI, and click **Assets** from the left pane.
1. Navigate to a folder, and select the desired asset.  
1. Click **Details** and select `Cr pin` from the rightmost pane. The C2PA metadata tab displays the following information about the asset.  
    1. **Generated Image:** Date and time in which the C2PA metadata was applied. 
    ...
    1. **About this C2PA metadata:** Name of the issuer along with the date and time of issuance. 
