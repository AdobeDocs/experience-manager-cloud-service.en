...
>[!NOTE]
>
>If the `dm_video` metadata schema is used in your environment, ensure that the same configuration is also applied to the `dm_video` schema. This ensures consistent behavior of Video Smart Crops across all video schema types.
...
1. In the metadata panel, set **[!UICONTROL Create Video Smartcrops]** to **Yes**, update the asset status to **[!UICONTROL Approved]**, and click **[!UICONTROL Save]**.
