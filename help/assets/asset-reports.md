...
AEM Assets provides several distinct reporting mechanisms for different purposes. Confusing one for another or misconfiguring the permissions needed to run them is the most common source of **[!UICONTROL My report is empty or stuck or missing]** error types.
...
* Excess `/libs` write access is a common misconfiguration to audit for proactively. This does not just fail to work, it can silently corrupt the report generation for sharing the affected group.
