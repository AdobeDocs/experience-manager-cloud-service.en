...
This guide covers the basics of Dynamic Media, how it works, what it can do for you, and what value it provides to your work and your customers.
...
>[!TIP]
>
>For best results, Adobe recommends that you read and view this Dynamic Media documentation on a desktop computer.
...
One of the main points to understand about Dynamic Media is the concept of _One primary asset file with multiple delivery options_.

To understand this concept better, consider the way you traditionally work with a single asset, such as an image or a video. You typically create one primary asset. Then, you manually create versions of that same asset for every experience, every device that is needed, every web page, and every property where it is used. Over time, that single asset can grow to 20, 30, or more versions, with no version history attached to them. Now, imagine doing that for every image or video that you have. The number of asset versions becomes overwhelming to maintain and update, not to mention the increase in storage costs.  

Dynamic Media, however, is fundamentally different from other systems because you use it to deliver your media _dynamically_ from single, primary assets, and from URL calls. The Dynamic Media URL paths that you request include instructions that tell the Adobe publish server how to display the asset when it is delivered to a customer's screen. For example, using the same single primary asset, you deliver it instantly in unlimited renditions, changing size, format, and resolution.

This unique delivery method ensures that consistent quality experiences are sent to any screen, regardless of size or bandwidth. Full-size videos are also optimized for all screen types and adaptively streamed to ensure a consistent, quality user experience.
...
As you continue, you learn more about why this concept of "one primary asset file, multiple delivery options" is important.
...
Adobe Dynamic Media technical support does not turn on Smart imaging by default because it requires a coordinated effort. Also, enabling Smart Imaging requires a complete clearing of your CDN cache, which is then refilled over time. To use Smart Imaging, submit a technical support ticket. Technical support then gives you a URL parameter that lets you try out Smart Imaging beforehand. You can try it on any of your web pages or images so you can see the performance that you get, and the savings. You can then have Smart Imaging turned on for your full site.
...
For example, on an iOS mobile device, it detects 4G, 5G, or Wi-Fi. Then, it automatically selects the right encoded video from among the various video bit rates within the Adaptive Video Set. The video is streamed to mobile devices, tablets, or desktop computers.
...
Using Adaptive Video Sets provides a smooth, high-quality playback for customers playing Dynamic Media video on multiple screens and devices. It simplifies video management.
...
The remaining 10% is HTML, CSS code, and specific tags. You want to optimize the 90% weight of that page and Dynamic Media helps with that effort. Earlier, you read about the concept of _one primary asset file with multiple delivery options_. This approach is significant in reducing overall page weight. The ability to take one primary asset and use it on a product detail page, a thumbnail page, your shopping cart, and your search grid, is highly efficient. And it ensures consistency across experiences as well.
...
Review the issues Dynamic Media is solving with the single-file approach and some of the solutions to that approach.
...
When you think about one file, you are creating an asset for every kind of experience. You have one starting image, and then you create 20, 30, or 40 variations of that image, which you store and pay for.

Then you make sure that the right image is used, which affects your ability to have consistency across your brands. And, if you cannot find an image, you have to go back in and duplicate those assets.

Dynamic Media lets you create variations of images, dynamically, from that one starting image. It lets you be creative with that primary asset and not have to coordinate repeatedly with your graphic design team or photo studio to create additional content. This results in reduced costs and time.
...
Dynamic Media's solution is to take that one primary video file, and let Dynamic Media make all the sizes through its transcoding process. Then, pair that with Dynamic Media's intelligent video player. This workflow guarantees that whether you use that video on your main landing page, or on a category or product detail page, it remains consistent throughout and delivers with high quality.
...
You begin the creation phase with the goal of producing your primary asset. These primary assets come from photo shoots, from video vendors, or from audio files that you have had created. You can use Adobe's Creative Suite applications such as Adobe InDesign, Adobe Photoshop, Adobe Illustrator to help you create the content. 
...
When you finish creating a primary asset, you upload it into Dynamic Media. The type of file you upload, and the format and size of the file are important attributes to Dynamic Media. At the time of upload, ensure you get the maximum value out of the one file support.

For example, the watch image below is 4560 x 3020 pixels. And while you may never use an image that size, you can still upload it. The larger the image, the better the quality that Dynamic Media can deliver, even down to a thumbnail rendition. Remember: you can easily _decrease_ the resolution of an existing image. But if you try to _increase_ the resolution of an image, the result is unsatisfactory.
...
Adobe recommends that you upload assets in a lossless format. It is best to avoid JPEG because when you deliver or save JPEG, you lose image quality over time. You want to start with the highest resolution images in a lossless format that is acceptable. That format is typically a TIFF or PNG file.

Regarding color space, when you think about a digital channel or web view, you typically think about RGB (Red, Green, Blue).

Most users do not consider delivering something in CMYK or why you want to deliver in CMYK. The reason is because that color space is most often used for delivering printed items. But, Dynamic Media can deliver in both color spaces.

There are many customers who still do print, such as warehouse wholesale clubs. And there are grocery stores who often print flyers on a weekly basis. Such customers require that their images be in both color spaces. Traditionally, that requires two different images: one RGB and one CMYK. However, you can upload CMYK assets directly into Dynamic Media, and have Dynamic Media deliver RGB assets through an image preset, or through a color profile, automatically. There is no need to create multiple versions of a file, thus maintaining the concept of _one primary asset file with multiple delivery options_.

<!-- **The Value of Renditioning??? or Demo portion** -->

### Publish and preview assets

After you upload assets into Dynamic Media, it is a good practice to _publish_ them by selecting the assets, and then clicking **[!UICONTROL Publish]** or **[!UICONTROL Quick Publish]** in Dynamic Media. Publishing assets is necessary if you intend to use them in any experience. After assets are published, they are available to include in a web page using a Dynamic Media-generated URL that you copy, or by embedding code on the page.
...
Part I of this documentation covered the basics of various Dynamic Media topics. If you want to learn more about what you read, use the materials below to explore concepts in greater detail. Otherwise, you can continue with Part II of this documentation. See [What's next in this Dynamic Media Journey](#whats-next). 
...
* [Use Dynamic Media with Experience Manager Assets](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/dynamic-media/dynamic-media-overview-feature-video-use)
...
In part II of this documentation, you closely examine Dynamic Media URLs to understand better what is going on when an asset is delivered. You also learn more about the fundamentals behind creating image presets to render assets, and learn about Image sets, Spin sets, and Mixed Media sets and how they are created. 

Go to [Dynamic Media: The Basics, Part II](/help/assets/dynamic-media/dm-journey-part2.md#dm-journey-d).
