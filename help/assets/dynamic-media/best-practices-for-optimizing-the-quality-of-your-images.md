...
By making informed choices about image formats and enabling Smart Imaging, you can impact performance and user experience.
...
By making these choices at the source level, you can contribute to the quality of your visual content.
...
In Experience Manager, you can sharpen images during ingestion, delivery, or both. Usually, however, it is best to sharpen images using only one method or the other, but not both. Sharpening images on delivery through a URL typically gives you the best results.
...
* Unsharp masking (`&op_USM`) &ndash; Unsharp masking is an industry standard sharpening filter. The best practice is to sharpen images with unsharp masking following the guidelines below. Unsharp masking lets you control the following three parameters:
  ...
  * `&op_sharpen=`amount,radius,threshold
    ...
    * **[!UICONTROL radius]** (0-250, width of the "sharpening lines" drawn around the sharpened object, as measured in pixels.)
    ...
    Note that the parameters radius and amount have opposing effects. Reducing radius can be compensated by increasing the amount. Radius allows finer control as a lower value sharpens only the edge pixels, whereas a higher value sharpens a wider band of pixels.
...
Gradually increase the amount from 1.75 to 4. If sharpening is still not satisfactory, increase the radius by a decimal point and run the amount again from 1.75 to 4. Repeat as necessary.
...
* As a best practice, set the `qlt= value` to 85.
...
The parameter `jpegSize` is useful. It guarantees that an image does not exceed a certain size for delivery to devices that have limited memory.
...
If the image requires further optimization, gradually fine-tune sharpening (unsharp masking) parameters by starting with a radius set to 0.2 or 0.3. Then, gradually increase the amount from 1.75 to a maximum of 4 (equivalent to 400% in Adobe Photoshop). Check to see that the desired result is achieved.

If sharpening results are still not satisfactory, increase the radius in decimal increments. For every decimal increment, restart the amount at 1.75 and gradually increase it to 4. Repeat this process until you achieve the desired result. While the values above are an approach that creative studios have validated, you can start with other values and follow other strategies. Whether the results are satisfactory to you or not is a subjective matter; therefore, structured experimentation is essential.

As you experiment, the following general suggestions are helpful to optimize your workflow:

* Test different parameters in real time on a URL.
* As a best practice, remember that you can group Dynamic Media Image Serving commands into an image preset. An image preset is URL command macros with custom preset names such as `$thumb_low$` and `&product_high$`. The custom preset name in a URL path calls these presets. Such functionality helps you manage commands and quality settings for different usage patterns of images on your website and shortens the overall length of URLs.
