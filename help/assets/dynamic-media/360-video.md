...
You can use the 360/VR Video viewer to render equirectangular video. The result is an immersive viewing experience of a room, property, location, landscape, medical procedure, and similar content.

Spatial audio is not currently supported; if audio is mixed in stereo, the balance (L/R) does not change as the user changes the camera viewing angle.

See [Using Dynamic Media 360 Videos and Custom Video thumbnail with AEM Assets](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/dynamic-media/video/dynamic-media-360-video-custom-thumbnail-feature-video-use#dynamic-media).

See also [Managing Viewer Presets](/help/assets/dynamic-media/managing-viewer-presets.md).

## 360 Video examples {#video-in-action}

Select [Space Station 360](https://s7d1.scene7.com/s7viewers/html5/Video360Viewer.html?asset=Viewers/space_station_360-AVS) to open a browser window and watch a 360° video. To change the viewing angle during video playback, move the pointer to a new location.
...
You can use Adobe Premiere Pro to view and edit 360/VR footage. For example, you can add logos, text, and equirectangular effects.
...
360 video assets that are uploaded into [!DNL Experience Manager] are labeled as **Multimedia** on an Asset page, similar to normal video assets.
...
1. Create a folder that is dedicated to your 360-video assets.
1. [Apply an adaptive video profile to the folder](/help/assets/dynamic-media/video-profiles.md#applying-a-video-profile-to-folders).
   ...
   Rendering 360-video content requires higher source video resolution and encoded renditions than standard non-360 video content.

   You can use the standard Adaptive Video Profile that is included with Dynamic Media. However, it results in perceptibly lower 360-video quality than non-360 video encoded with the same settings and rendered with a non-360 video viewer. Therefore, if high-quality 360 video is required, do the following:
    ...
    * Ideally, your original 360-video content has either one of the following resolutions:
        ...
        * 2160p - 3840 x 2160, known as 4k, UHD, or Ultra HD resolution. This large display resolution is most often found on premium television sets and computer monitors. The 2160p resolution is referred to as "4k" because the width is approximately 4000 pixels. Specifically, it offers four times the pixels of 1080p.
   ...
   This approach increases the network and CPU usage of the device.
...
You can use Preview to see how your 360 Video appears to users. You can also verify that it is functioning correctly.
...
See [Linking URLs to your web application](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md). The URL-based method of linking is not possible if your interactive content has links with relative URLs, such as links to [!DNL Experience Manager Sites] pages.
...
1. On the preview page, near the upper-left corner of the page, select the drop-down list, then select **[!UICONTROL Viewers]**.
    ...
    * To alter the viewing angle of the static scene, move the pointer across the video.
    * To begin playback, select the video's **[!UICONTROL Play]** button. To alter your viewing angle as the video plays, move the pointer across the video.
    ...
    * From the Viewers list, select **[!UICONTROL Video360VR]**.
        ...
        Virtual Reality (VR) video is immersive video content that users access using virtual reality headsets. As with ordinary videos, you create VR videos at the beginning when a video is being recorded or captured using 360° video cameras.
...
See also [Linking URLs to your web application](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md). The URL-based method of linking is not possible if your interactive content has links with relative URLs, including links to [!DNL Experience Manager Sites] pages.
