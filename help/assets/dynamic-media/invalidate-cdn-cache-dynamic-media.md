...
*Part 1: Creating a CDN Invalidation template*
...
1. On the **[!UICONTROL CDN Invalidation Template]** page, do one of the following options based on your scenario:
    ...
    | I need to create a template. What do I enter? | In the **[!UICONTROL Create Template]** text field, enter an image URL (including image presets or modifiers) referencing `<ID>`, instead of a specific image ID as in the following example:<br><br>`https://my.publishserver.com/is/image/company_name/<ID>?$product$`<br><br>If the template just contains `<ID>`, then Dynamic Media fills in `https://<publishserver_name>/is/image/<company_name>/` where `<publishserver_name>` is the name of your Publish Server, `<company_name>` is the name of your company and `ID` is the selected assets to invalidate.<br><br>Only images&ndash;that is, `/is/image`&ndash;can be auto formed based on the template. For `/is/content/`, adding assets such as videos or PDFs using the asset picker does not auto generate URLs. Instead, you must specify such assets either in the CDN Invalidation template, or you can manually add the URL to such assets in *Part 2: Setting CDN Invalidation options*.<br>Asset formats that are supported in Dynamic Media are eligible for invalidation. Assets such as PowerPoints or plain text files are not supported assets for CDN invalidation.<br>When you create the template, but sure you pay careful attention to syntax and typos; Dynamic Media does not do any template validation.<br>**Important:** Each entry in a CDN Invalidation template must be on its own line.<br>The following template example is for illustration purposes only.
...
1. On the **[!UICONTROL CDN Invalidation]** &ndash; **[!UICONTROL Add Details]** page, select the assets for CDN invalidation.
   ...
   | Option | Description |
   | --- | --- |
   | **[!UICONTROL Invalidate asset associated image presets in CDN]** | (Optional) When you check this option, you can select one or more Dynamic Media assets&ndash;regardless of MIME type&ndash;and their associated image presets for cache invalidation.<br>Assets and their associated pre-defined preset URLs are auto formed for invalidation. This option works only for image assets.|
   | **[!UICONTROL Invalidation based on template]** | (Optional) Check this option to use only the defined template for URL formation. |
   | **[!UICONTROL Add Assets]** | Use the Asset Picker to select assets you want to invalidate. You can select either published or unpublished assets.<br>You may get a blank list when you add assets. Dynamic Media uses the CDN Invalidate Template to automatically create URL to invalidate the CDN. If no CDN Invalidate Template was created, then you get a blank list.<br>Caching at the CDN is URL-based, not asset-based. Therefore, it is necessary for you to be aware of the complete URLs that are on your website. After you determine those URLs, you can add them to the template.Then, you can select and add those assets and invalidate the URLs in one step. The other option you have is to add URLs to the CDN Invalidation list. If you prefer this approach, it is not necessary to select and add assets before you tap **[!UICONTROL Next]** and then **[!UICONTROL Submit]** for invalidation.<br>Use this option in conjunction with **[!UICONTROL Invalidate asset associated image presets in CDN]**, or **[!UICONTROL Invalidation based on template]**, or both. |
   | **[!UICONTROL Add URL]** | Manually add or paste full URL paths to Dynamic Media assets whose CDN cache you want to invalidate. Use this option if you did not create a CDN Invalidation Template in ***Part 1: Working with the CDN Invalidation Template***, and have only a few assets to invalidate.<br>**Important:** Each URL that you add must be on its own line.<br>You can invalidate up to 1000 URLs at a give time. If the number of URLs in the **[!UICONTROL Add URL]** text field is greater than 1000, you are unable to tap **[!UICONTROL Next]**. In such cases, you must tap **[!UICONTROL X]** to the right of a selected asset or a manually added URL to delete it from the invalidation process.|
...
1. On the **[!UICONTROL CDN Invalidation]** &ndash; **[!UICONTROL Confirm]** page, in the **[!UICONTROL URLs]** list box, you will see a list of one or more URLs generated from the CDN Invalidation Template you created earlier and the assets you just added.
    ...
    For example, with the CDN Invalidation Template previously created, suppose you added single asset named `spinset`. When you tap **[!UICONTROL Tools > Assets > CDN Invalidation]** it results in the following five generated URLs in the **[!UICONTROL CDN Invalidation - Confirm]** user interface:

    ![CDN Invalidation - Confirm](/help/assets/assets-dm/cdn-invalidation-confirm-2.png)

    If necessary, tap **X** to the right of a URL if you want to delete it from invalidation process.
