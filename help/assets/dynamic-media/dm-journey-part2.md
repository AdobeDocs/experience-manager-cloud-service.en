...
After your Dynamic Media assets are uploaded and published, you can copy an asset's generated URL and paste it into your browser to see how the asset appears to a customer. The following copied URL for a watch image breaks down by color to make it easier to read and understand.

![Anatomy of a Dynamic Media URL](/help/assets/dynamic-media/assets/dm-colored-url.png)
_Structure of a Dynamic Media URL._

The first part of the URL in red is referencing the server domain itself. In this case, Dynamic Media is running on a generic server domain, which is `https://s7d1.scene7.com/is/image/`. You can easily identify if Dynamic Media serves a set of images by checking the server domain. The URL structure is consistent. There are, however, some Dynamic Media customers that have switched over to a dedicated server domain where it is `name-of-your-company.scene7.com`. A dedicated server domain is required for Smart Imaging.

The account name is the portion in purple. In this case, the account is called `jpearldemo`.

The asset ID or name, `AdobeStock_28563982` is in green. Notice that the asset has _no_ file extension such as `.png` or `.jpg`. When assets are ingested into Dynamic Media, the file extension is stripped out and a different kind of file is created: a pyramid-TIFF file. The pyramid-TIFF allows Dynamic Media to create renditions quickly on-the-fly.
...
With your browser window still open to the Dynamic Media URL and the watch image, look at how you can create renditions of the image by changing the URL.
...
Notice that a new rendition of the watch is generated. A key takeaway to understand from this simple exercise of changing the image's width is that the image seen is generated 100% dynamically.
...
The moment you press **[!UICONTROL Enter]**, the browser goes back to the Dynamic Media Image Server. It generates a rendition of the watch, based on the new width value you entered, then delivers the image to the browser and caches it. 
...
Now try adding a rotation parameter to the watch image. At the end of the URL path, immediately following `wid=1000`, type `&rotate=90`, and then press **[!UICONTROL Enter]**. [Try it](https://s7d1.scene7.com/is/image/jpearldemo/AdobeStock%5F28563982?wid=1000&rotate=90){target="_blank"}.
...
Again, the moment you press **[!UICONTROL Enter]**, a new rendition of the watch generates immediately. You can see the performance levels, which explains why Dynamic Media can deliver more than 800,000 image requests, _per second_, during high-traffic periods.

Changing image processing parameters in a URL on an image-by-image basis is inefficient, especially for websites with thousands of images. A much better approach is using image presets.  

## Fundamentals of creating image presets to render assets {#dm-journey-e}

There are multiple ways to create an image. You want to have an image available. Traditionally, a creative professional goes into Adobe Photoshop, and saves each of these different renditions as static images.

![Static images](/help/assets/dynamic-media/assets/dm-static-images.png)
_Good: static images, each one manually created._

Now imagine the creative lead looks at the images and says, 
...
The creative reshoots all new static images.
...
You want that image delivered with the same parameters wherever they are used.
...
Selecting the **[!UICONTROL Advanced]** tab gives you options for the asset's color space. Depending on the format selected in the **[!UICONTROL Basic]** tab, you can deliver assets in RGB, Grayscale, or CMYK. Use the **[!UICONTROL Color Profile]** pull-down menu to select how to deliver a CMYK image asset for print. Notice, too, that there are additional parameters you can apply for sharpening your images. In this case, **[!UICONTROL Unsharp Mask]** was applied.
...
You recall in [Structure of a Dynamic Media URL](#dm-journey-d) earlier, that you read about the Dynamic Media URL and how that is built. The **[!UICONTROL Image Modifier]** text box is where you can type any additional image processing parameters that you want. The parameters get included in the preset name of the URL when your images are delivered, using the preset. In the screenshot above, the parameter `bgc=451B15` was added. That is, a dark brown background color was added.

You can think of an image preset as a configuration for your images. It delivers any images that use the preset consistently every time; it is the same. The parameter `&op_brightness=+10` was also added to increase the brightness slightly.  
...
_To generate a rendition of an image, apply the image preset Medium._

To check the appearance of the image, copy the URL and paste it into your browser. [Try it](http://s7d1.scene7.com/is/image/jpearldemo/AdobeStock_74043302?$Medium$){target="_blank"}. 
...
But what if you have to change an image on your website? For example, suppose you have done some testing, and found that the image of 120 x 120 (the `Cart` image preset) is not being perceived as you thought. Increase the image size by increasing the width to 175 pixels and the height to 175 pixels. Traditionally, you go into Adobe Photoshop and re-create all of those cart images. But with Dynamic Media, you edit the image preset by updating the Width and Height values to 175 and save your preset, as seen in the example below.
...
After you change your image preset, and clear the cache, all the images get updated, and all the URLs that are being used with that preset, do _not_ change anywhere. That means no broken links and no webpage redirects are necessary.

## Image sets, Spin sets, and Mixed Media sets {#dm-journey-f}

Some of the more popular uses of Dynamic Media are the ability for you to create Image sets, Spin sets, and Mixed Media sets.
...
After you create an Image set, Mixed Media set, Spin set, or other interactive media, preview how it appears and behaves for a customer. Dynamic Media has numerous built-in viewers that let you do that.

You begin by selecting the image set to open it in a preview, as the following example shows.
...
In this case, the `Flyout` viewer was selected. At this point, you can preview the image set in the viewer. But, it is best to see it in your browser as a customer sees it. You select **[!UICONTROL URL]** in the lower left, then copy the URL and paste it into your browser. [Try it](https://s7d1.scene7.com/s7viewers/html5/FlyoutViewer.html?asset=jpearldemo/Running&config=jpearldemo/Flyout){target="_blank"}.   

The single URL lets you use the image set and viewer where you need them on your website. You noticed in the previous example that **[!UICONTROL Embed]** is to the right of the URL button. By selecting **[!UICONTROL Embed]**, you can copy the code for this image set/viewer, and add it into a web page or an Experience Manager Sites component.

The Flyout viewer is a default, standard viewer whose properties you can edit. Or, like creating an image preset, you can create your own, custom viewer.

Now, suppose your sales and marketing team does not prefer the Flyout viewer. They like the zoom feature but they want customers to see the zoom effect directly over the shoes. In such a case, you apply the InlineZoom viewer to the image set, and copy and paste its URL into your browser to see how it behaves. [Try it](https://s7d1.scene7.com/s7viewers/html5/FlyoutViewer.html?asset=jpearldemo/Running&config=jpearldemo/InlineZoom){target="_blank"}.

When you move the mouse pointer over the shoe, the image zooms in, and you can see more detail as you move the pointer. This zoom effect is due to the size of the image that was initially uploaded into Dynamic Media.

As you consider your role as a consumer, or as you work in your professional role, and as you visit different websites, you see examples of these features. Consider how that is being done, and how you can use the capabilities of Dynamic Media in your own work and on your company's website. 

You read about image sets and viewers. Let's look at a couple of other viewers and try them out on single assets. To reset the viewer, click the **[!UICONTROL Refresh]** button in the lower-left corner.
...
To learn more about what you read, use the materials below to explore concepts in greater detail. Otherwise, your Dynamic Media Journey is complete!
