...
Dynamic Media with OpenAPI capabilities put DAM at the core of an agile and efficient content supply chain ecosystem to ensure asset governance and delivery.
...
Before submitting a request to enable Dynamic Media with OpenAPI capabilities on AEM as a Cloud Service, ensure that it is not already enabled. To verify if it is enabled, execute the following steps:

1. To be confirmed from engineering and Product Management
