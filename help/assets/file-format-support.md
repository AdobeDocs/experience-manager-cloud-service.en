...
|---|---|---|
| ZIP files sent through Dynamic Media processing profiles | The processing step fails explicitly with an unsupported-format error. This causes the workflow instance to accumulate in an unhealthy or retrying state at scale. | Exclude ZIP (and other archive or 3D package files) from Dynamic Media processing profiles using the asset selection criteria so the profile is never applied to non-media file types in the first place. |
| AVIF images| The file is stored in the Digital Asset Management (DAM), but AEM does not process it and no thumbnail or preview is generated. | AVIF is not a supported input format for asset processing in AEM as a cloud service. Convert to JPG or PNG before uploading, if a preview or rendition is required. |
...
Out-of-the-box MIME type restrictions apply globally across DAM by default, but folder-specific restrictions are also possible. Configure the allowed MIME types for a specific folder, for example, `/content/dam/projects` through **[!UICONTROL Tools]** > **[!UICONTROL Assets]** > **[!UICONTROL Assets Configuration]**, listing only the MIME types that should be accepted for that folder. All other types, including common ones like Excel spreadsheets, are then blocked for that folder specifically without affecting rest of the DAM.
...
* Enable and configure the workflow purge maintenance so that completed and stale workflow instances are cleaned up automatically rather than accumulating.
