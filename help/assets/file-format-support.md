...
Adobe Experience Manager (AEM) Assets processing has two distinct mechanisms that behave differently when a file type is not fully supported: standard renditions (always requested for every asset, non-configurable) and Dynamic Media / custom processing profiles (configurable, and can be scoped by MIME type or asset selection criteria). Knowing which one is failing makes it easier to determine whether the behavior is expected or there is something to fix.
...
| *SVG* files missing the offset attribute on **[!UICONTROL <stop>]** elements| Dynamic Media's image server returns HTTP 403 through **[!UICONTROL /is/image]** for that specific file (while **[!UICONTROL /is/content]** still renders it), because the *SVG* violates the SVG 1.1 specification.| Validate SVGs against the SVG 1.1 specification (for example, with the W3C Validator) before uploading, and re-export from the source tool (for example, Illustrator) with SVG 1.1 compliance.|
| Files with MIME types outside the configured allow-list (for example, *.pem* certificates)| Upload is rejected in the UI. | AEM Assets can technically store any binary file, but an organization's asset upload restrictions may limit the accepted MIME types. Update the allowed MIME type list (globally, or per folder) to include the required type. |
| Metadata values in a bulk CSV import that start with or contain reserved characters (**[!UICONTROL #]**, **[!UICONTROL /]**, **[!UICONTROL ;]**, **[!UICONTROL \]**, **[!UICONTROL []**, **[!UICONTROL ]]**, **[!UICONTROL %]**, **[!UICONTROL {]**, **[!UICONTROL }]**, **[!UICONTROL ?]**, **[!UICONTROL &]**, pipe symbol)| The parser skips or ignores the affected metadata entry without a hard failure.| Remove or replace the reserved characters in metadata values before running a bulk CSV metadata import. |
...
3. If a specific file fails while similar files succeed, suspect the file itself first. Check for restricted or licensed fonts (*PPTX* / *PDF*), SVG specification compliance, or an unsupported format or extension mismatch (*AVIF*, *JFIF*) before assuming an AEM configuration issue.
