...
When AEM Assets is integrated with the Document-Based Authoring tools, such as Microsoft Word or Google Docs, it provides an asset selector in your editor. Use this asset selector to access to the AEM Assets, and insert approved assets into your document.
If you already have an Edge Delivery Services website, see [AEM Assets plugin](https://github.com/adobe-rnd/aem-assets-plugin/blob/main/README.md) document to learn how to integrate AEM Assets with your existing AEM project. 
Follow the following [Prerequisites](#integrate-aem-assets-with-microsoft-word-and-google-docs) and [Integrating AEM Assets with Document-Based Authoring environment](#integrate-aem-assets-with-microsoft-word-or-google-docs-to-use-aem-assets-with-microsoft-word-or-google-docs) sections if you do not have an Edge Delivery Services website to publish your AEM Assets inclusive content authored in document based authoring tools.
...
* Integrate AEM with a Document-Based Authoring tool to set up the authoring environment. See [Getting Started - Developer Tutorial](https://www.aem.live/developer/tutorial) to learn how to set up the authoring environment.
...
Select the following links to learn how to use Dynamic Media with OpenAPI capabilities to deliver images, videos and other asset types in your content:
...
See [WKND Travel](http://bit.ly/3DExLnf), a site that is built using the Document-Based Authoring capabilities of Edge Delivery Services. The site's content is authored in [Google Docs](https://drive.google.com/drive/folders/1HCCHRWp4HJIXW_cUv5cRDQ5DzzqiZsXT) and Dynamic Media with OpenAPI capabilities is used to deliver assets in the content. After authoring, the content is published from the document directly. Explore this [Git repository](https://github.com/hlxsites/franklin-assets-selector/tree/aem-dynamicmedia-demo/blocks) to know about all the essential files, folders, configurations, website's styling and functionality codes that are used to create the Document-Based Authoring setup for this Edge Delivery Services (EDS) site.

## Integrating AEM Assets with Universal Editor based authoring flows for Edge Delivery Services {#integrate-aem-assets-with-UE-universal-editor}
