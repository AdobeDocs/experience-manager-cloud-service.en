...
* **On Time** - The date and time at which the published page is made visible (rendered) on the publish environment. The page must be published, either manually or by pre-configured auto-replication.
  ...
  * If already [published,](/help/sites-cloud/authoring/sites-console/publishing-pages.md) this page is available on the publish instance, but kept dormant (hidden) until rendering at the specified time.
  * If not published and [configured for auto-replication,](/help/operations/replication.md#on-and-off-times-trigger-configuratio) the page is automatically published, then rendered, at the specified time.
