...
* **Significance**: Evaluating if your challenger is really better than the control. Calculating significance allows you to rule out luck and concentrate on the results that have a real effect.

### Experiment variants and general workflow {#experiment-variants-workflow}

In order to start creating experiment variants you need to get the experimentation plugin working, as detailed on the Experimentation page (hyperlink TBD). 
...
Explain the elements below, mention side-kick EDS
...
By convention, it is recommended to create a folder with a lowercase experiment ID in your `/experiments/ folder` (for example /experiments/ccx0076/). All the pages for the challenger variants are located in this folder.
...
Preview and publish the challenger page using the sidekick and when you are done authoring the challenger page, the URL of the published challenger will be used in the next section - configuring the experiment. (REWRITE)

### Configure your experiment {#configure-experiment}

As soon as you have your challengers ready to go, all you need to do is to go back to the control page and add metadata indicating that this page is now part of the test.
...
You can override the traffic split by configuring the metadata. For more information, see the following [page](https://github.com/adobe/aem-experience-decisioning/wiki/Experiments#authoring)

ADD METADATA PARAMETERS HERE, DO WE DETAIL THIS FURTHER?
...
You use a full page experiment to test between two variants of the same page. Basically, this is a full page variant of an a/b test. You will replace the whole content of a page (which becomes the control page) in one go and create a challenger variant with different content. Keep in mind that by default the customer traffic is split evenly (50/50), but you can create custom splits if you like.

METADATA SETUP

#### Sections of the page eperiment {#sections-of-the-page}

This is experiment is similar to the full page one but now the a/b test will contain changes to a section of the page instead of the whole content. For example, you can modify and test a carousel element, the call to action element and so on.

METADATA SETUP

#### Multi-path (page?) Experiment {#multi-path}
...
Note that the above assumes you have different content variants to serve, but if you want to run a pure code-based a/b Test, this is achievable via:
...
You can create browser based experiments, where you deliver separate challenger pages depending on the browser used. You can, for example, serve a different challenger page to a Firefox user as opposed to a Chrome user. This is achieved by leveraging the audience feature in the experiments plug-in. 
...
Before you start authoring this experiment variant, the audience parameter needs to be defined in the project codebase. For more details, see [Link to git].
...
6.While in preview mode you should see the experiment button in green. This button lets you switch between the page variations.
7.Select all the pages and click Publish from the side-kick to push both the control and the variants live.
