...
The AEM as a Cloud Service Developer Console includes a set of read-only tools for debugging in Cloud environments. It can be accessed through a per-environment link in Cloud Manager.
...
Using the Developer Console, developers can access the following features: 
