...
* **Config** - Configure settings for various features on your AEM environment.
  * See the [Config Pipeline article](/help/operations/config-pipeline.md) for a list of supported configurations, which includes log forwarding, purge-related maintenance tasks, and various CDN configurations, and to manage them in your repository so they are deployed properly.
