...
A non-production pipeline serves to run code quality scans or to deploy source code to a development environment.
...
Pipelines also differ by the code they deploy, beyond production and non-production environments.
...
* Back-End Code - Immutable content (Java code, OSGi configurations, repoinit) and mutable content
...
Full-stack pipelines deploy web tier configurations if a [web tier config pipeline](#web-tier-config-pipelines) is not configured.
...
* To include or ignore the Dispatcher configuration, the user can configure the full-stack pipeline if the corresponding web tier config pipeline for the environment does not exist.
...
Front-end code is any code that is served as static files. It is separate from UI code served by AEM and includes site themes, customer-defined SPAs, SPAs, and other solutions. 

Front-end pipelines help your teams streamline your design and development process by enabling accelerated deployment of front-end code, asynchronously of back-end development. This dedicated pipeline deploys JavaScript and CSS to the AEM distribution layer as a theme, resulting in a new theme version, which is referenced from pages delivered by AEM.
...
See [Develop Sites with the front-end pipeline](/help/implementing/developing/introduction/developing-with-front-end-pipelines.md) for how this process works along with some considerations to be aware of to maximize the potential of this process.

## Web tier config pipelines {#web-tier-config-pipelines}

Web tier config pipelines allow exclusive deployment of HTTPD/Dispatcher configuration to the AEM runtime, decoupling it from other code changes. It is a streamlined pipeline that provides users who want to deploy only Dispatcher configuration changes an accelerated means to do so.
