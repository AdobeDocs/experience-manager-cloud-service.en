...
1. In the **Edit Program** dialog box, use the tabs to set the various options you want.
   ...
   * To enable or disable the Web Application Firewall (WAF) at any time, select the **Security** tab, then check or uncheck the **WAF-DDOS Protection** check box. Checking this box activates the feature, but beyond some automatic CVE protection, you must deploy the WAF rules through Cloud Manager for full protection. If WAF rules are licensed but this check box is not checked, the feature is not active. For more information,see [Traffic Filter Rules including WAF Rules](/help/security/traffic-filter-rules-including-waf.md).
