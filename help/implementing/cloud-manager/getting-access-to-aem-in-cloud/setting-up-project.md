...
* There must be a `pom.xml` file in the root of the git repository. This `pom.xml` file references as many sub-modules (which in turn have other sub-modules, and so on) as necessary.
...
* Cloud Manager discovers deployable content packages by scanning for content package `.zip` files, which are contained in a directory named `target`. Any number of sub-modules produce content packages.
* Cloud Manager discovers deployable Dispatcher artifacts by scanning for `.zip` files (also contained in the directory named `target`), which have directories named `conf` and `conf.d`.
...
And if you want to output a simple message only when the build is run outside of Cloud Manager, do the following:
...
>[!NOTE]
>
>Deploy artifacts from password-protected Maven repositories with caution, as Cloud Manager does not evaluate this code with its [code quality rules](/help/implementing/cloud-manager/custom-code-quality-rules.md). This method should be used only for specific situations and applied only to code not related to AEM. Adobe recommends including both the Java sources and the entire project source code along with the binary. This ensures greater transparency and maintainability throughout the deployment process.
...
* With this file in place, reference the server ID from inside a `<repository>` and/or `<pluginRepository>` element inside the `pom.xml` file.
* These `<repository>` and `<pluginRepository>` elements are included within a [Cloud Manager-specific profile](#activating-maven-profiles-in-cloud-manager), though their inclusion is not strictly required.

For example, assume the repository is at `https://repository.myco.com/maven2`, the username Cloud Manager uses is `cloudmanager`, and the password is `secretword`. Take the following steps:
...
It is recommended to deploy the Java sources alongside the binary to a Maven repository. 
...
It is recommended to deploy the whole project source alongside the binary to a Maven repository. Doing so lets you rebuild the exact artifact. 
 
Configure the maven-assembly-plugin in your project as follows:
...
The mechanism to set this property depends upon the way the build produces the content package. For example, with the `filevault-maven-plugin` you configure the plug-in as follows.
...
When an execution is started, the current HEAD commit for the branch pipeline is extracted. The commit hash is visible in the UI and via the API. When the build step completes successfully, the resulting artifacts are stored based on that commit hash and are reused in subsequent pipeline executions.
...
* [Maven version handling](/help/implementing/cloud-manager/managing-code/project-version-handling.md) replaces the project version only in production pipelines.
    If the same commit is used for both a development deployment and a production pipeline, and the development deployment runs first, the versions are deployed to staging and production unchanged. However, a tag is still created in this case.
