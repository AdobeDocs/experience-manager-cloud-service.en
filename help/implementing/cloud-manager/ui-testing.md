...
>
>The file may contain a line *DO NOT MODIFY*". It is simply a legacy warning from older templates/samples and does *not* block you from making the opt-in edits required for Cloud Manager UI testing. You can safely ignore the advisement. That is, you may edit `assembly-ui-test-docker-context.xml` and `pom.xml` in *your project* when following the opt-in steps (for example, to include `testing.properties`).
