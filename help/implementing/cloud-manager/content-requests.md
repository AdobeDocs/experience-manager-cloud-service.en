...
| URL for customers to monitor their CS program | Excluded | Recommended URL to externally monitor the availability.<br><br>`/system/probes/health`|
...
| Exclude CIF Commerce Calls | Excluded | These are requests made to AEM that get forwarded to the Commerce Integration Framework&mdash;the URL starts with `/api/graphql`&mdash;to avoid double counting, they are not billable for Cloud Service.|
...
| Exclude favicon.ico | Excluded | For customers with SAML auth, the favicon request made by browsers is delivered as HTML and counted as CR. Filtering out favicon.ico is recommended.|
