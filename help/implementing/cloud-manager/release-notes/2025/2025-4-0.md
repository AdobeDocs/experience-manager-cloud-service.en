...
* **(UI) Domain validation enhancement**
    ...
    When adding a domain, Cloud Manager displays an error if it is already installed in a Fastly account: "*Domain already installed in Fastly. Remove it before adding to Cloud Service.*"

## Private beta program {#private-beta-program}

To get exclusive access to upcoming features before their general release, participate in Cloud Manager's private beta program.

The following private beta opportunity is currently available:
...
>[!NOTE]
>
>Currently, the default pull request code quality checks are exclusive to GitHub-hosted repositories, but an update to extend this functionality to other Git vendors is in the works.
...
* **Enforced uniqueness for Advanced Networking port forward names**  
    ...
    Cloud Manager now enforces unique naming for Advanced Networking port forwards. Previously, duplicate names were allowed, which led to conflicts. This fix ensures that each port forward entry has a distinct name, aligning with best practices for network configuration integrity. <!-- CMGR-67082 -->
