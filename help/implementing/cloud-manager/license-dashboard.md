...
Content Requests have variances with an organization's Analytics reporting tools as summarized in this table.

See also [Understanding Content Requests](/help/implementing/cloud-manager/content-requests.md).

|Reason For Variance|Explanation|
|---|---|
|Tagging|All pages that are tracked as AEM content requests may or may not be tagged with Analytics tracking. All API calls that are tracked as AEM content requests are not tagged by an organization's Analytics tool.<br>Pages or API calls may be tagged to track actions or unique page views instead of all views.|
