...
1. Set `X-AEM-Edge-Key`. The value should be configured using a Cloud Manager config pipeline first and then same edge key should be configured in customer CDN, as described in [this article](/help/implementing/dispatcher/cdn-credentials-authentication.md#CDN-HTTP-value).
