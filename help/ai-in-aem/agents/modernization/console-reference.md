...
The Experience Modernization Console is a hosted AI-assisted development environment for Edge Delivery Services, exposed as a web interface at [`aemcoder.adobe.io`](https://aemcoder.adobe.io). After connecting to their GitHub project, users can immediately start prompting changes in natural language without any further setup or local environment configuration.

Currently, interested users can request access to ensure a smooth onboarding experience.
...
* Content synchronization with Document Authoring
...
Enterprise governance: Developers retain full control over what ships. All changes made through the Console require review and approval before deployment, ensuring governance, brand consistency, and security.
...
* **Plan / Execute modes** (light bulb and wizard icons): Toggle between planning and execution modes
  * **Plan mode**: The AI analyzes requests and outlines an approach without making changes. Useful for understanding strategy before committing
...
* **Clear chat**: Resets the conversation and clears the AI's context window. Use when starting a new task unrelated to the previous conversation

## Content View {#content-view}

The Content view (file icon in the left sidebar) provides tools for browsing and previewing content.

* **Expand / Collapse file browser** (» or « chevrons): Toggle the file browser panel
* **Preview / HTML toggle** (Preview and HTML icons): Switch between viewing the rendered HTML preview and the underlying document authoring content structure
