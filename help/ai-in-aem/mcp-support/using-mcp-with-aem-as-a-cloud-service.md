...
This step is performed by each user (or by an administrator of the MCP client application, where supported). Configuration details vary slightly between applications, but the general process is:
...
* Configure a new MCP connection pointing to the AEM MCP server URL(s)
* Sign in using your Adobe ID when redirected
* Use Copilot experiences that can call AEM MCP tools as part of broader solutions.
...
*"Using the configured AEM tools, create a new content fragment for the spring campaign based on our hero banner model and fill in its fields from this brief."*
